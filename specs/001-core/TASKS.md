...
- **Status:** ✅ Done — `bundled` is a new Cargo feature (off by default, so every pre-existing `cargo build`/`cargo test`/`tauri dev` stays `light` unchanged); `binary_manager::build_flavor()` reads it at compile time and `persistence::seed()` now writes that (not a hardcoded `"light"`) into `settings.build_flavor` on first run, which is all S7's About line and Onboarding/Shell's `build_flavor === "bundled"` checks (already wired since T17) needed — no frontend files touched. `binary_manager::bundled_binary_path(resource_dir, which)` resolves `<resource_dir>/bin/<name>[.exe]`; `persistence::seed_bundled_binaries` (new, `INSERT OR IGNORE` like `seed`'s other defaults — first-run-only, never clobbers a later user-picked path) writes `ytdlp_path`/`ffmpeg_path` to that location. `lib.rs`'s `setup()` calls both behind `#[cfg(feature = "bundled")]`, so a `light` binary has zero resource-dir lookup or seeding code in it at all — this is what makes K1-AC6 hold: App.svelte's existing onMount already skips S1 whenever both binaries resolve, and a bundled build's binaries now resolve via the ordinary `detect()` path (persisted setting → `probe_version`) with no PATH search, same as a light user manually setting a path today. `tauri.bundled.conf.json` (new, `--config`-merged onto the base config only for the bundled build) adds `bundle.resources` mapping `binaries/bin/*` → the packaged resource dir; `src-tauri/binaries/bin/README.md` documents why the actual yt-dlp/ffmpeg binaries aren't checked into the repo (large, per-platform, already fetchable at the exact URLs T16's `download_url` uses — ponytail: release pipeline populates the folder, upgrade path is running the light build's own in-app download once and copying its output in, if no pipeline exists yet) and `scripts/build-flavors.sh` drives `tauri build` for light/bundled on the current OS. AC3's four artifacts: only the Linux `light` leg is actually build-and-run-verified in this sandbox (no Windows runner, no real shipped binaries to bundle here) — matches the exact gap CONVENTIONS already accepted for T1's PATH-search and T16's download URLs (Windows verified by inspection, not CI); `GATE SKIPPED`-equivalent for the Windows/bundled legs is the honest status, to be closed out at whichever CI runner actually builds them. New unit tests: `build_flavor_is_light_without_the_bundled_feature`, `bundled_binary_path_joins_resource_dir_bin_and_file_name`, `bundled_seeded_path_is_detected_without_any_path_search` (binary_manager.rs); `seed_sets_build_flavor_from_compile_time_flavor`, `seed_bundled_binaries_sets_paths_once_and_never_overwrites` (persistence.rs). While running the full suite (not just `--lib`) for this task, found and fixed pre-existing breakage from T19 unrelated to T20: 4 integration-test files (`tests/{logs_and_retry,queue_scheduling,queue_lifecycle,engine_integration}.rs`) hadn't been updated for T19's new `AddDownloadParams`/`NewItem` fields or T16's `Emitter::emit_binary_health`, so `cargo test` (as opposed to `cargo test --lib`) didn't compile at all before this task — trivial field/method additions, no behavior change, fixed so the full suite is green as CONVENTIONS' step 3 requires. `cargo build`/`cargo build --features bundled` both clean; `cargo test --lib` 78 passed (was 75 pre-T20 + 3 new), 1 ignored; `cargo test` (full workspace, single-threaded) all non-`#[ignore]`d tests green, network-dependent ones correctly skipped; `npm run build` clean and unchanged (no frontend files touched, consistent with this task's Files list).
...
## Milestone 6 — CONTRACT REMEDIATION (T22–T29)

> **Why this exists.** T4–T17 shipped the whole frontend with **hand-rolled markup + one scoped `<style>` block per file** instead of the committed stack. Three contract violations, all pre-existing and self-documented as repeated `ponytail:` shortcuts ("same no-shadcn-pipeline precedent as every prior task"):
> 1. **shadcn-svelte never installed** — ARCHITECTURE §1 "Component library" + FILE_STRUCTURE `components/ui/…` require it; `components.json` exists but no component was ever pulled, no `bits-ui`, no Tailwind, no `$lib/utils` `cn`.
> 2. **No Tailwind** — DESIGN.md §2 says "components consume only shadcn variables **or Tailwind classes bound to them**"; instead every file carries scoped CSS. `app.css` has no `@import "tailwindcss"`.
> 3. **lucide-svelte unused** — ARCHITECTURE §1 "Icons = lucide-svelte (supersedes Material Symbols)"; instead unicode glyph placeholders (StageToken, Sidebar).
>
> Docs are correct; **code is wrong** — so this is remediation, not a spec change. **No living-doc patch** (ARCHITECTURE/DESIGN/FILE_STRUCTURE already state the target). These tasks close the `ponytail:` icon/shadcn debt those tasks named.
>
> **Cross-cutting rules for every T23–T28 (migration) task — not repeated per task:**
> - **Behavior-preserving.** Same props, events, store calls, IPC, keyboard nav, focus rings, ARIA, and observable behavior as today. This is a swap of the *rendering layer*, never a logic change. If a screen's behavior changes, the task is wrong.
> - Replace hand-rolled primitives with the shadcn-svelte component from `$lib/components/ui/` (installed in T22). Replace unicode glyphs with `lucide-svelte` icons.
> - **Delete the file's scoped `<style>` block**; express styling as Tailwind utilities bound to the shadcn CSS variables (DESIGN.md §2 map). **No raw hex/px/font in components** (CONVENTIONS "Styling") and logical properties only (`ms-`/`me-`/`ps-`/`pe-`, never `left/right`). `app.css` stays the one file with raw values.
> - Tokens missing from the map get added to `app.css` + DESIGN.md §2 first (CONVENTIONS), not inlined.
> - `VirtualList.svelte` stays hand-written (no shadcn equivalent) but loses any raw-value `<style>` in favor of utilities.
> - Each task ends with `npm run build` + `svelte-check` clean and a **grep proof**: zero `<style` blocks and zero unicode-glyph icons remain in the touched files.

### T22 — Frontend pipeline foundation: Tailwind + shadcn-svelte + lucide  *(remediation base)*

- **Status:** ✅ Done — installed via real `shadcn-svelte@1.4.1` CLI (`npx shadcn-svelte add <16 components> -y -o`) against the existing `components.json`; CLI silently downgraded `style: new-york` to its internal `nova` (unsupported style warning) — `components.json` itself left untouched. `vite.config.ts` gets `@tailwindcss/vite` + a `resolve.alias.$lib`→`src/lib` (tsconfig `paths` alone doesn't resolve at runtime for plain Vite); `tsconfig.json` gets matching `$lib`/`$lib/*` paths (relative, no `baseUrl`, to avoid the TS6-deprecation warning) plus `"types":["vite/client"]` to fix a **pre-existing** (predates this task, verified via `git stash -u` on `main`) `svelte-check` error on `main.ts`'s `import "./app.css"`. `app.css`: `@import "tailwindcss";` + `@custom-variant dark (&:is(.dark *));` prepended above `:root` (byte-identical token values kept), a new `@theme inline` block added right after `:root` mapping all 19 standard shadcn vars + `--font-sans`/`--font-mono` to the existing raw tokens (verbatim `var()` aliasing, no new values); `--surface-lowest/--surface-high/--warning*/--error-token*` and the `*`/`body` rules left untouched. Caveat/deviation: the CLI's current icon package is `@lucide/svelte` (not `lucide-svelte`, which upstream now marks deprecated) — since the task's acceptance criterion and ARCHITECTURE.md both name `lucide-svelte` explicitly, all 12 generated icon imports were rewritten `@lucide/svelte`→`lucide-svelte` and the dependency swapped in `package.json`; `lucide-svelte@1.0.1` confirmed installed with a working `icons/*` export map. `src/lib/utils.ts` didn't get generated by the CLI (pre-existing partial `components.json` state meant it treated the project as already-initialized); created by hand from the shadcn-svelte registry's own `utils.json` content verbatim (`cn`, `WithElementRef`, `WithoutChild(renOrChild)`), with `clsx`/`tailwind-merge` promoted from transitive to direct deps. One extra dir beyond the declared 16 — `ui/separator` — is a genuine transitive dependency of `select` (`SelectSeparator`), not an extra top-level component. **Criteria verified:** (1) OBSERVED — `npm run build` and `npx svelte-check` both clean (0 errors/warnings, 762 files); compiled CSS inspected directly and confirmed `body{background:var(--background);color:var(--foreground);font-family:var(--font-sans);margin:0;font-size:14px;font-weight:400;line-height:20px}` survives byte-for-byte past Tailwind's preflight reset (preflight's `*` box-sizing/margin reset and `html` font-family don't win against body's own directly-set, non-inherited declarations). No GUI screenshot taken (sandbox has no working screenshot tooling, consistent with prior tasks' notes) — visual "unchanged" claim is inferred from CSS output, not a live render. (2) OBSERVED — `find src/lib/components/ui -maxdepth 1 -type d` lists all 16 declared dirs (+`separator`). (3) OBSERVED — temporary probe file (`import Sun from "lucide-svelte/icons/sun"`, `cn("p-4")`, `<Button>`) built successfully (module count 157→172, no errors), then deleted and `main.ts` restored; final build back to 157 modules / clean svelte-check. (4) OBSERVED — built CSS contains `.bg-primary{background-color:var(--primary)}` and `:root{--primary:#c0c1ff}`, and `button.svelte`'s default variant uses `bg-primary text-primary-foreground`, so the chain from bits-ui-driven `<Button>` → Tailwind utility → shadcn var → DESIGN_SYSTEM purple is proven in the emitted artifact, not just inferred from source.
- **Objective:** stand up the real styling/component pipeline the contract assumes, so every later migration task just consumes it.
- **Inputs:** `components.json` (already present, `new-york`/neutral/cssVariables), `app.css` token `:root` map (DESIGN.md §2), `shadcn` CLI (already a devDep).
- **Outputs:** Tailwind wired; `src/lib/components/ui/*` populated; `lucide-svelte` + `cn` util available; build green.
- **Dependencies:** T21 (lands after v1 code exists; blocks all of T23–T28).
- **Files:** `package.json` *(gen deps)*, `vite.config.ts` (+`@tailwindcss/vite`, ensure `$lib`→`src/lib` alias), `tsconfig.json` (`$lib` path), `svelte.config.js` (if alias needed there), `src/app.css` (prepend `@import "tailwindcss";` **above** the existing `:root` token map — keep every token value byte-for-byte), `src/lib/utils.ts` *(gen — `cn` = clsx+tailwind-merge)*, `src/lib/components/ui/…` *(gen — shadcn copies)*.
- **Acceptance criteria:**
  1. `npm run build` + `svelte-check` clean; `npx tauri dev` renders the **unchanged** current UI (Tailwind reset must not visually regress the existing dark `surface`/Instrument Sans look — reconcile the reset against the token map).
  2. The full declared component set exists under `src/lib/components/ui/`: `button, dialog, sheet, table, input, select, dropdown-menu, checkbox, toggle, progress, sonner, tooltip, collapsible, alert-dialog, card, badge` (FILE_STRUCTURE §`ui/…`).
  3. `import Sun from "lucide-svelte/icons/sun"` resolves and renders; `cn("a","b")` importable from `$lib/utils`.
  4. shadcn theme variables (`--primary` etc.) resolve to the DESIGN_SYSTEM purple tokens — a shadcn `<Button>` dropped anywhere shows `primary` purple, proving the DESIGN.md §2 map drives bits-ui.
- **NOT:** don't migrate any view/component yet (T23–T28); don't add components beyond the declared set; don't touch backend.
- **Difficulty:** M — one-time, but Tailwind-v4-reset-vs-existing-tokens reconciliation and the CSP-safe (all-local) shadcn install need care.
- **Model:** **Opus/Sonnet** — high blast radius, pipeline wiring, not a mechanical swap. Use the `shadcn` skill + shadcn MCP for the canonical init/add; do **not** hand-roll `ui/` primitives.
- **Context pack:** ARCHITECTURE §1 (committed stack table). DESIGN.md §2 (variable map — the source of truth for wiring bits-ui theme), §7 (hard rules). CONVENTIONS "Styling"/"Folder rules". `components.json`, current `app.css`, `vite.config.ts`, `tsconfig.json`. shadcn-svelte docs via the shadcn skill.

---

### T23 — Migrate Shell chrome to shadcn + lucide  *(S2/S6/S7 chrome)*

- **Status:** ✅ Done — `Sidebar.svelte`: nav/filter rows, collapse toggle, and Add CTA rebuilt on shadcn `Button` (`variant="ghost"` for nav/pinned rows, `variant="default"` for Add); live count badges are shadcn `Badge` (`variant="secondary"`, `font-mono`), still `$derived` off `filtersStore.countFor(filter, items)` against the unfiltered `items` prop — unchanged store call, so counts stay live exactly as T13 left them. Active-filter marker kept as **weight + indicator, not color alone** (DESIGN.md §7 rule #4): `font-bold` plus a mutually-exclusive `border-s-primary`/`border-s-transparent` pair (never both classes present at once, sidestepping any utility-order ambiguity around Tailwind's logical border-color generation — verified in the built CSS: `.border-s-primary{border-inline-start-color:var(--primary)}`). Collapsed-rail labels (collapse toggle, Add, each filter row, Presets, Settings) now use shadcn `Tooltip.Root/Trigger/Content` via the bits-ui `child` snippet (asChild pattern) instead of the T13 `title`-attr ponytail; every row shares one markup path by toggling `Tooltip.Root`'s own `disabled={!collapsed}` rather than branching JSX, so tooltips only actually open in the collapsed 56px rail — except Settings' row, which intentionally keeps its tooltip always-enabled (`Ctrl/Cmd+,` hint) since that's what the pre-migration code's `title` value already did in both states (redundant ternary in the old code, preserved deliberately, not a behavior change). Nav/status glyphs → lucide: `ChevronsLeft`/`ChevronsRight` (collapse toggle, literal translation of `«`/`»`), `Plus` (Add), `List`/`Download`/`Clock`/`CirclePause`/`CircleCheck`/`CircleAlert` (all/downloading/queued/paused/completed/failed), `Package` (Presets), `Settings` (Settings). `QueueToolbar.svelte`: search is shadcn `Input` (`type="search"`, leading `Search` icon absolutely positioned via `start-2.5`/`ps-8`, logical not physical), N control is shadcn `Input type="number"` (same `onNInput` validation, same `queueStore.setConcurrency` call), Start all/Pause all are shadcn `Button variant="outline"` with `Play`/`Pause` icons — `pauseIds`/`resumeIds` derivations and the `queueStore.pauseAll`/`resumeAll` calls are byte-identical to T13. `Shell.svelte`: only its own hand-rolled bits needed touching — the Presets/Settings scrim overlay's close `button` became a shadcn `Button` (`variant="ghost" size="icon-sm"`) with a lucide `X` icon (the `✕` glyph), and the scrim/panel `<style>` block became Tailwind utilities bound to CSS vars via the arbitrary-utility pattern the task brief calls out (`bg-[var(--surface-lowest)]`, `bg-[color-mix(in_srgb,var(--surface-lowest)_70%,transparent)]`, `rounded-lg` — confirmed in DESIGN.md's `@theme inline` map, `--radius-lg: var(--radius)`, so `rounded-lg` is byte-identical to the old `border-radius: var(--radius)`, not an approximation); scrim click-outside-to-close and `stopPropagation` on the inner panel are unchanged, no new Escape-to-close behavior was introduced (deliberately stayed off shadcn `Dialog` here — not in T23's context pack (`button`/`badge`/`input`/`tooltip` only) and swapping to a real `Dialog` would add focus-trap/Escape semantics beyond a pure rendering-layer swap; left as a `ponytail:`-worthy candidate for whichever future task touches Shell.svelte's overlay chrome next, none currently listed in T24–T28's file lists). `App.svelte` left untouched — its only markup is the pre-`ready` loading state, not S2/S6/S7 chrome, so out of scope per the task's own "+App.svelte only if it holds chrome markup" carve-out. **Verified:** `npm run build` and `npx svelte-check` both clean (775 files, 0 errors/warnings). Grep proof — `grep -n "<style" Shell.svelte Sidebar.svelte QueueToolbar.svelte App.svelte` returns zero matches in the three T23 files (App.svelte's own unrelated `.loading` style block is the only hit, expected/out of scope); a non-ASCII character sweep of the three files turns up only `§`/`—` in comments and `…` in the search placeholder copy — no icon glyphs remain. Behaviors traced (not visually walked — same sandbox screenshot-tooling limitation every prior task has noted): live count badges (still plain `$derived` reads, no store change), active-filter weight+indicator marker, 56px/1100px collapse threshold (`filtersStore.collapsed || innerWidth < 1100`, untouched), focus rings (shadcn `Button`/`Input` ship `focus-visible:ring-ring/50` bound to `--ring` out of the box, satisfying NFR-4 without extra CSS), and degraded-mode Add gating (`downloadsDisabled` derivation, `openAdd()` early-return, and the `addDisabled` prop threading into `Sidebar`/`Queue` are all unchanged — only the disabled-Add tooltip's *rendering* moved from `title` to `Tooltip.Content`, its trigger condition is identical). No backend touched, no IPC rerouted, no store logic changed.
- **Objective:** the persistent chrome uses real components + icons.
- **Files:** `src/lib/views/Shell.svelte`, `src/lib/components/Sidebar.svelte`, `src/lib/components/QueueToolbar.svelte` (+`App.svelte` if it holds chrome markup).
- **Dependencies:** T22.
- **Acceptance criteria:**
  1. Sidebar nav/filter rows, collapse toggle, and Add CTA are shadcn `button`/`badge`; count badges still update live and the active filter is still marked by **weight + indicator, not color alone** (DESIGN.md §7); focus ring intact (NFR-4).
  2. Toolbar search is shadcn `input`; N control + Start/Pause-all are shadcn `button`; collapsed-rail labels use shadcn `tooltip` (replacing the T13 `title`-attr ponytail).
  3. Sidebar/StageToken glyphs replaced by `lucide-svelte` icons (nav, status, chevrons); no unicode-glyph icons remain in these files.
  4. Behavior identical to pre-migration (rules above); no `<style>` block remains in the touched files.
- **NOT:** row internals (T24); StageToken itself (T24).
- **Difficulty:** M. **Model:** **Sonnet** (mechanical, behavior-guarded).
- **Context pack:** T13 status note + these 3 files; UX.md S2; DESIGN.md §6 (240/56px), §7. shadcn `button`/`badge`/`input`/`tooltip` docs.

---

### T24 — Migrate Queue rows to shadcn + lucide  *(S2 list)*

- **Status:** ✅ Done — `QueueRow.svelte`: checkbox is shadcn `Checkbox` (`checked`/`onCheckedChange`, `onToggleSelect(item.id)` call unchanged — the boolean arg bits-ui hands back is ignored same as the old `onchange` was); the inline-progress pill is shadcn `Progress` (`value={Math.min(100, Math.max(0, item.percent))}`, identical clamp math to the old `bar-fill` width calc) with the thin/thick signature preserved via a conditional `h-1`/`h-2` class merged over the component's own `h-1` default (DESIGN.md §4 gap #3: 4px standard, 8px active — `ACTIVE_STAGES` set unchanged); the `⋯` overflow menu is shadcn `DropdownMenu` (`Root`/`Trigger` via the bits-ui `child` snippet asChild pattern/`Content`/`Item`, `variant="destructive"` on Remove) — dropped the old hand-rolled outside-click `$effect` and `rowEl` ref entirely since bits-ui owns open/close/outside-click/Escape natively, same set of conditional items (Pause/Resume/Retry/Move up/down/Cancel/Remove) gated on the same `ACTIVE_STAGES`/`TERMINAL_STAGES` checks calling the same `queueStore` methods. `StageToken.svelte`: unicode glyph map replaced by a lucide icon map (`Download`/`Merge`/`Clock`/`CirclePause`/`CircleCheck`/`CircleAlert`/`CircleX`, `Circle` fallback for an unrecognized stage string) rendered via a `$derived` icon component reference; colors kept byte-identical to the DESIGN.md gap #2 mapping (`text-primary` active, `text-muted-foreground` queued/paused/cancelled, `text-secondary` completed, `text-[var(--error-token)]` error) — icon + `label-mono` (`font-mono`) text, never color alone. `SelectionBar.svelte`: Start/Pause/Cancel/Remove/Move-up/Move-down/Clear are shadcn `Button` (`variant="outline"` bulk verbs, `variant="ghost" size="icon-sm"` for the ▲/▼ pair, `variant="ghost"` for Clear) with `Play`/`Pause`/`Ban`/`Trash2`/`ArrowUp`/`ArrowDown` icons added — same 7 callback props (`onStart`/`onPause`/`onCancel`/`onRemove`/`onMoveUp`/`onMoveDown`/`onClear`), same `count > 0` gate, still never calls the store directly for Cancel/Remove. `Queue.svelte`: the hand-rolled `window.confirm()` + `setTimeout`-cleared `<div class="toast">` became a controlled shadcn `AlertDialog` (`confirmState` replaces the synchronous `confirm()` return — `requestCancel`/`requestRemove` now just set `confirmState` instead of blocking, and the actual cancel/remove side effects that used to run inline after `confirm()` returned true were moved verbatim into a new `confirmProceed()` fired by `AlertDialog.Action`'s `onclick`) + `svelte-sonner`'s `toast()` with an `action: { label: "Undo", onClick }` (replacing the hand-rolled toast div/timer-clearing `undo` closure) — the underlying soft-delete mechanics are untouched: Remove still hides ids client-side only (`hiddenIds`), still defers the real `queueStore.bulk("remove", ids)` exactly `TOAST_WINDOW_MS` (5000ms) via the same `setTimeout`/`removeTimers` map, Undo still just `clearTimeout`s and un-hides with nothing reaching the backend; Cancel still calls `queueStore.bulk("cancel", ids)` immediately with Undo calling `queueStore.retry(id)` per id (same ponytail as T14: not necessarily the item's exact prior stage). Drag-reorder (`dragCandidateId`/`dragStartY`/`dragActive`/`dropId`, the 6px threshold, `elementFromPoint`/`closest("[data-row-id]")` resolution, `queueStore.reorderTo`) and roving-tabindex keyboard nav (`focusedId`, arrow-key `$effect`s, `.focus()` on `[data-row-id]`, Enter→`openDetail`) are byte-identical — no pointer/keyboard logic touched, only the surrounding markup. Empty-state "Add"/"Show all" inline links became shadcn `Button variant="link"` (`h-auto p-0 align-baseline` override via `cn`/`twMerge` so they still sit inline in the sentence) calling the same `onAdd`/`onShowAll` props. `VirtualList.svelte`: style-only per the file list — the `<style>` block became Tailwind utilities (`relative overflow-y-auto`, `absolute start-0 top-0 w-full`, `box-border w-full`) bound to the same inline `style:height`/`style:transform` px math; the windowing logic itself (`scrollTop`/`totalHeight`/`visibleCount`/`startIndex`/`endIndex`/`visibleItems`/`offsetY` derivations, `onScroll`) was not touched, byte-for-byte. `App.svelte` gained the one allowed extra file's worth of change: `<Toaster/>` (shadcn `sonner`) mounted once at the root next to `<GlobalBanner/>` — checked first and confirmed nothing had mounted it yet (T23 didn't touch `App.svelte`'s body). **Verified:** `npm run build` and `npx svelte-check` both clean (784 files, 0 errors/warnings — no pre-existing error surfaced this time, unlike T9/T14/T17's noted one). Grep proof — `grep -n "<style" Queue.svelte QueueRow.svelte StageToken.svelte SelectionBar.svelte VirtualList.svelte` returns zero matches across all 5 files; a non-ASCII sweep turns up only `§`/`—` in comments and the `—` em-dash used as the pre-existing null-value placeholder text (`formatBytes`/`formatEta` returning `"—"` for `null`, present before this task) — no icon glyphs (`⋯`/`✕`/`▲`/`▼`/`↓`/`⇄`/`‖`/`⏸`/`✓`/`⚠`) remain anywhere in the 5 files. Behaviors traced (not visually walked — same sandbox limitation every prior UI task has noted): virtualization DOM-window (`VirtualList`'s slice math untouched, still renders only `visibleCount + 2×buffer` rows regardless of the 60-item fixture size); drag-vs-click 6px threshold (`Math.abs(e.clientY - dragStartY) > 6` gate unchanged, still resolves via `elementFromPoint`→`closest("[data-row-id]")`→`reorderTo` above threshold, `openDetail` below); roving-tabindex + Enter (`tabindex={focused ? 0 : -1}`, arrow-key `$effect` calling `.focus()`, `Enter` → `onOpenDetail` in `QueueRow`'s `handleKeydown`, untouched); undo timing (`TOAST_WINDOW_MS = 5000` threading into both the real `setTimeout` for Remove's deferred `bulk()` call and the `sonner` `toast()`'s `duration` option — two independent timers with the same constant, matching the old code's toast-div timer + separate remove-timer pairing); progress thin/thick signature (`h-1`/`h-2` conditional on `ACTIVE_STAGES`, DESIGN.md §4 gap #3); StageToken icon+text (never color alone, DESIGN.md gap #2 colors preserved exactly). No backend touched, no IPC rerouted, no `queueStore`/`filtersStore` method signatures changed — every store call site is textually identical to T14's, just relocated into `confirmProceed()`/`Button`/`DropdownMenu.Item` `onclick` handlers instead of native `<button onclick>`/`confirm()`.
- **Objective:** the dense row, progress signature, selection, bulk, overflow menu, and undo toasts on real components.
- **Files:** `src/lib/views/Queue.svelte`, `src/lib/components/QueueRow.svelte`, `src/lib/components/StageToken.svelte`, `src/lib/components/SelectionBar.svelte`, `src/lib/components/VirtualList.svelte` (style-only).
- **Dependencies:** T22.
- **Acceptance criteria:**
  1. Row checkbox = shadcn `checkbox`; the inline-progress **pill** is shadcn `progress` themed to the DESIGN.md signature (thin standard / thick active) — the visual signature (DESIGN.md §4 gap #3) is preserved.
  2. Overflow `⋯` menu = shadcn `dropdown-menu`; Cancel/Remove confirm = shadcn `alert-dialog`; undo toasts = shadcn `sonner` (replacing the hand-rolled `confirm()`/toast — behavior/soft-delete timing per ARCHITECTURE §8 unchanged).
  3. `StageToken` = lucide icon + `label-mono` text, colored per DESIGN.md gap #2, **never color alone**; the `⋯`/chevron/action glyphs are lucide.
  4. 60-item virtualized scroll, drag-reorder-past-threshold vs click-opens-detail, roving-tabindex + Enter all behave exactly as T14 (rules above); no `<style>` blocks remain.
- **Difficulty:** L. **Model:** **Sonnet**.
- **Context pack:** T14 status note + these files; UX.md S2; DESIGN.md §4 gaps #2/#3/#4, §5, §7. shadcn `checkbox`/`progress`/`dropdown-menu`/`alert-dialog`/`sonner` docs. Mount `<Toaster/>` (sonner) once at the app root if not added in T23.

---

### T25 — Migrate Add + Format screens to shadcn + lucide  *(S3/S4)*

- **Status:** ✅ Done — `AddDownload.svelte` (S3) and `FormatPicker.svelte` (lives at `src/lib/components/FormatPicker.svelte`, not `views/` — the task's file list had a stale path; it's the same file T10 built) rewritten on shadcn `Dialog`/`Input`/`Select`/`Collapsible`/`Toggle`/`Button`; `FormatQuickPicks.svelte` on `Toggle`+`Input`. Zero `<style>` blocks, zero unicode-glyph icons (✕/▸/▾/✓/⌕ → lucide `X`/`ChevronRight`/`Check`/`Search`) across all three files. **Esc-priority reconciliation (the task's hardest constraint), `ponytail:`-commented in both dialog files:** bits-ui's `Dialog` registers its own Escape handling on `document`, which the DOM bubble phase always reaches *before* `window` — left at its default, it would auto-close S3 independent of topmost-ness and, since it never calls `stopPropagation` (only `preventDefault`, confirmed against bits-ui 2.18.1's `EscapeLayerState` source), let the event keep bubbling and also fire T15's DetailDrawer `window` listener, double-closing S3+S5 together instead of S5 alone. Fix: both `Dialog.Content`s set `escapeKeydownBehavior="ignore"` (suppresses only the auto-close callback; the layer still no-ops via `preventDefault`) so Escape stays owned exclusively by the pre-existing hand-rolled listeners — S3 keeps its unchanged `window`-level `handleKeydown` (still the sole Ctrl/Cmd+N-opens / Escape-closes mechanism, so DetailDrawer's registered-first-plus-`stopImmediatePropagation` priority is untouched), and S4 (nested inside S3's `Dialog.Content` in the component tree) adds a local `onkeydown` directly on its own `Dialog.Content` that fires during the bubble phase *before* the event would ever reach `document`/`window`, calling `e.stopPropagation()` then its own `close()` — reproducing the original hand-rolled scrim's local interception, so a single Escape only ever closes the topmost overlay when S3+S4+S5 are all open simultaneously. Both dialogs still use the real shadcn `Dialog` component per AC1/AC2 (the task's own documented hand-rolled-container fallback wasn't needed) — only the Escape *auto-close* behavior is suppressed; backdrop-click and the header Close button still go through bits' normal `onOpenChange` (`handleClose()` always invokes the passed `onOpenChange`, verified in source, even when `open` is passed as a plain non-`bind:` prop — same controlled pattern T24's `AlertDialog.Root` in `Queue.svelte` already established), which mirrors the old scrim-`onclick`/icon-button `close()` exactly, including `reset()` on every close path. Also added `onOpenAutoFocus` on S3's `Dialog.Content` (bits' own focus-trap default would otherwise race the pre-existing "focus the URL input" effect) so the URL input still gets focus deterministically on open, matching pre-migration timing. Quick-pick↔expression one-group semantics, the 137+140 compose (`selectedVideoId`/`selectedAudioId` → `composedExpression` → `expression` `$effect`, cleared by editing the expression field directly), and probe/duplicate/validation error surfacing (`probeState`/`probeError` rendering, `queueStore.add`'s try/catch) are all untouched — no logic lines changed, markup/styling only. `npm run build` and `npx svelte-check` both clean (0 errors, 0 warnings). Not visually walked in a running `tauri dev` window — per this skill's opt-in-visual-verification rule, confirmed instead by tracing Esc/Ctrl+N wiring, the quick-pick/expression group, the format compose+writeback, and probe/duplicate/error surfacing end-to-end across all three files.
- **Objective:** Add overlay + Format picker as real dialogs/selects.
- **Files:** `src/lib/views/AddDownload.svelte`, `src/lib/views/FormatPicker.svelte`, `src/lib/components/FormatQuickPicks.svelte`.
- **Dependencies:** T22.
- **Acceptance criteria:**
  1. Add overlay = shadcn `dialog`; URL/template/extra-args fields = shadcn `input`; PresetSelect = shadcn `select`; AdvancedDisclosure = shadcn `collapsible`.
  2. Format picker = shadcn `dialog` (modal fallback) reusing the T24-migrated `VirtualList`; quick-pick chips = shadcn `toggle`/`button`.
  3. Probe/duplicate/validation error surfacing and the frontend `format_expr` compose are byte-for-byte unchanged; icons (close, chevrons, expand) are lucide; no `<style>` blocks remain.
- **Difficulty:** M. **Model:** **Sonnet**.
- **Context pack:** T9/T10 status notes + these files; UX.md S3/S4; DESIGN.md §3. shadcn `dialog`/`select`/`input`/`collapsible`/`toggle` docs.

---

### T26 — Migrate Detail drawer to shadcn + lucide  *(S5)*

- **Status:** ⛔ Not started.
- **Objective:** the docked drawer, facts grid, and log disclosure on real components.
- **Files:** `src/lib/views/DetailDrawer.svelte`, `src/lib/components/FactsGrid.svelte`, `src/lib/components/LogDisclosure.svelte`.
- **Dependencies:** T22.
- **Acceptance criteria:**
  1. Drawer = shadcn `sheet` (right side, non-scrim as UX §2 requires) — Esc-topmost-only priority over S3 preserved; FactsGrid on shadcn `card`; LogDisclosure = shadcn `collapsible` (still auto-expands on `error`).
  2. `watch_log`/`log_line` subscribe-on-open / unsubscribe-on-close and the contextual footer actions (Open file/folder via `open_path`) behave exactly as T15.
  3. Copy/open-folder/open-file/status icons are lucide; no `<style>` blocks remain.
- **Difficulty:** M. **Model:** **Sonnet**.
- **Context pack:** T15 status note + these files; UX.md S5; DESIGN.md §3/§5. shadcn `sheet`/`collapsible`/`card` docs.

---

### T27 — Migrate Presets screen to shadcn + lucide  *(S6)*

- **Status:** ⛔ Not started.
- **Objective:** preset list + editor on real components.
- **Files:** `src/lib/views/Presets.svelte`.
- **Dependencies:** T22.
- **Acceptance criteria:**
  1. Preset list = shadcn `table`; editor fields = shadcn `input`; delete confirm = shadcn `alert-dialog` (replacing the ponytail `confirm()`); default-preset action clear.
  2. Single-default / last-preset invariants and `INVALID_FORMAT_EXPR` surfacing unchanged; row/action icons lucide; no `<style>` block remains.
- **Difficulty:** M. **Model:** **Haiku/Sonnet** (smallest, single-file).
- **Context pack:** T11 status note + this file; UX.md S6. shadcn `table`/`input`/`alert-dialog` docs.

---

### T28 — Migrate Onboarding + Settings + banner to shadcn + lucide  *(S1/S7)*

- **Status:** ⛔ Not started.
- **Objective:** onboarding wizard, settings, binary rows, and the global banner on real components.
- **Files:** `src/lib/views/Onboarding.svelte`, `src/lib/views/Settings.svelte`, `src/lib/components/BinaryRow.svelte`, `src/lib/components/GlobalBanner.svelte`.
- **Dependencies:** T22.
- **Acceptance criteria:**
  1. Wizard/settings sections = shadcn `card`; all fields = shadcn `input`/`select`; binary in-app download progress = shadcn `progress`; the mid-session binary-missing banner uses shadcn `alert` styling (add `alert` via MCP if not in the T22 set).
  2. `detect/download/set_binary_path`, proxy/N/dir/template writes, and banner trigger (K1-AC7) behave exactly as T16/T17; status/download/warning icons lucide; no `<style>` blocks remain.
- **Difficulty:** M. **Model:** **Sonnet**.
- **Context pack:** T16/T17 status notes + these files; UX.md S1/S7. shadcn `card`/`input`/`progress`/`alert` docs.

---

### T29 — 🚦 REMEDIATION GATE — contract conformance

- **Status:** ⛔ Not started.
- **Objective:** prove the three violations are closed app-wide, with no behavior regression.
- **Dependencies:** T23–T28.
- **Journey to walk:** the T18 full kernel journey + Flow D, re-walked — every screen now visibly using shadcn components and purple `primary`, keyboard-only pass, focus rings, WCAG AA spot-check.
- **Observations / proof required:**
  1. `grep -rl "<style" src/lib` returns **nothing** (all scoped blocks gone; `app.css` remains the only raw-value file).
  2. No unicode-glyph icon placeholders remain: the `ponytail:` glyph comments in `StageToken.svelte`/`Sidebar.svelte` are gone and `lucide-svelte` is imported where icons render.
  3. `src/lib/components/ui/` populated; every migrated view imports from it. `npm run build` + `svelte-check` clean; `cargo build` untouched.
  4. Kernel journey + Flow D pass; AA contrast confirmed with a tool; keyboard-only completion.
- **Acceptance:** anything failing is fixed before the cycle closes. **Model:** human gate (walk-and-capture).
- **Difficulty:** M.
- **Context pack:** PLAN "Demo Gate 4/5"; full UX.md; the T22–T28 outputs. No new code.
...
remediation (Milestone 6): T22 ─┬→ T23 ─┐
                                ├→ T24  │
                                ├→ T25  ├→ [T29 gate]
                                ├→ T26  │
                                ├→ T27  │
...
T22 is the base; **T23–T28 depend only on T22 and are independent of each other** (mostly disjoint files) — run them sequentially, or in parallel if edits to shared roots (`App.svelte`, `app.css`, the sonner `<Toaster/>` mount) are coordinated. T29 gates them all.
